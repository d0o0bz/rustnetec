...
- **进程级归属识别**：每一条 TCP、UDP、QUIC 连接都能追溯到所属进程。Linux 使用 eBPF，macOS 使用 PKTAP，Windows 使用 ETW 并在不可用时自动回退到 IP Helper，FreeBSD 则走原生 API。详情会显示 PID、支持平台上的 PPID、可执行文件、用户/组名称和匹配可信度。Wireshark 与 tcpdump 做不到这一点；`netstat` / `ss` 也无法展示实时状态。
...
# 需要先安装 Npcap(https://npcap.com/dist/)，并启用 "WinPcap API-compatible Mode"
# 安装时可取消勾选 "Restrict Npcap driver's access to Administrators only"，
# 普通用户即可直接抓包（仅驱动安装需要一次性管理员提权）
